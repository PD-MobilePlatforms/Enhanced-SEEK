 ...
 */
/*
 * Contributed by: JVL Ventures, LLC.  This is an updated version of the original contribution by Giesecke & Devrient GmbH.
 */
...
import org.simalliance.openmobileapi.service.security.gpac.dataobjects.APDU_AR_DO;
...
import org.simalliance.openmobileapi.service.security.gpac.dataobjects.NFC_AR_DO;
...
public class AraControlDB {
	...
	final private static String TAG = "AccessControlDB";
    ...
    AraControlDB(AccessRuleApplet mApplet) throws AccessControlException, CardException {
    	this.mApplet = mApplet;
    	getAllAccessRules();
    	
    	// keep the current value of refresh tag
    	mRefreshTag = mApplet.readRefreshTag();
    }
    
    public ChannelAccess getAccessRuleFromAllArCached( byte[] aid, Certificate[] appCerts ) throws AccessControlException, CardException, CertificateEncodingException {
    	...
    	if( mRefreshTag != tag ) {
    		// if refresh tag differs -> retrieve all access rules.
    		mRuleCache.clear();
    		getAllAccessRules();
    		mRefreshTag = tag;
    	}
        	
    	channelAccess = findAccessRuleInAllArCached( aid, appCerts );
    ...
    private void getAllAccessRules() throws AccessControlException, CardException {
    	
    	android.util.Log.v(TAG, "Retrieving All Access Rules");
    	
    	Response_ALL_AR_DO all_ar_do = mApplet.readAllAccessRules();
    	
    	ArrayList<REF_AR_DO> ref_ar_do_array = all_ar_do.getRefArDos();
    		
    	for (int i = 0; i < ref_ar_do_array.size(); i++) {
    		REF_DO ref_do = ref_ar_do_array.get(i).getRefDo();
    		AR_DO ar_do = ref_ar_do_array.get(i).getArDo();
    			
    		mapArDo2ChannelAccess(ref_do, ar_do);
    	}
    ...
    private ChannelAccess findAccessRuleInAllArCached( byte[] aid, Certificate[] appCerts ) {
    	...
    	for( Certificate appCert : appCerts ){
			...
			try {
		        ...
		        channelAccess = mRuleCache.get(ref_do);
		        if (channelAccess != null) break;
		        
			} catch (CertificateEncodingException e) {
				throw new AccessControlException("Problem with App Certificate.");
			}
    	}
    	// return if AR is found
    	if( channelAccess != null ) return channelAccess;
