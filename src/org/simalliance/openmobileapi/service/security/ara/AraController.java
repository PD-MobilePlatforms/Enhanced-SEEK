...
public class AraController {
    ...
    // various Access Rule retrieve mechanisms supported
    public enum ArRetrieveMechanisms {
    	CACHE_ALL_RULES,
    	//INSTANT_QUERY,  // Not implemented yet
    	INSTANT_QUERY_WITH_CACHE;
    }
    private ArRetrieveMechanisms mMechanism;
    
    public ArRetrieveMechanisms getMechanism() {return mMechanism;}
    ...
    public AraController(AccessController master) {
    	mMaster = master;
    	mMechanism = ArRetrieveMechanisms.CACHE_ALL_RULES;
    	//mMechanism = ArRetrieveMechanisms.INSTANT_QUERY_WITH_CACHE;
    }
    ...
    protected ChannelAccess internalEnableAccessConditions(IChannel channel, byte[] aid,
        ...
        // If mechanism is to retrieve all rules
        if ( mMechanism == ArRetrieveMechanisms.CACHE_ALL_RULES ) {
        	// when db is null, construct a new db and retrieve all the access rules
	        if( mAccessControlDB == null ) { 
	        	mAccessControlDB = new AraControlDB(new AccessRuleApplet(channel));
	        } else {
		        // set new applet handler since a new channel is used.
		    	mAccessControlDB.setApplet(new AccessRuleApplet(channel));
	        }
        } else {
        // mechanism is instant query
        ...
        else // SE Access Control
        {
            ...
            try {
            	...
            	if ( mMechanism == ArRetrieveMechanisms.CACHE_ALL_RULES ) {
            		channelAccess = mAccessControlDB.getAccessRuleFromAllArCached(aid, appCerts);
            	} else {
            		channelAccess = mAccessControlDB.getAccessRule(aid, appCerts);
            	}
