...
public class AccessRuleApplet {
    ...
    final private static CommandApdu mGetAll = new CommandApdu(0x80, 0xCA, 0xFF, 0x40, 0x00);
    ...
    public synchronized Response_ALL_AR_DO readAllAccessRules() throws AccessControlException, CardException {
        ...
        if( response.isStatus( 0x9000 ) ){
			...
			arLen = BerTlv.getEncodedLength(response.getData(), 0);
			...
			try {
				tempTlv = Response_DO_Factory.createDO(stream.toByteArray());
				
				if( tempTlv instanceof Response_ALL_AR_DO ){
					return (Response_ALL_AR_DO)(tempTlv);
				} else {
					throw new AccessControlException("GET ALL AC Rules returned invalid Tlv.");
				}
			} catch (ParserException e) {
				throw new AccessControlException("GET ALL AC Rules not successfull. Tlv encoding wrong.");
			}
    ...
    public synchronized byte[] readSpecificAccessRule( byte[] aid_ref_do ) throws AccessControlException, CardException {
    ...
    public synchronized long readRefreshTag() throws AccessControlException, CardException {
